...
public class Fishing {
    ...
    public static enum FishingSpots {
        ...
        private static Map<Integer, FishingSpots> fishingSpots = new HashMap<>();
    ...
    public static boolean canFish(Player p, FishableData.Fishable fish, boolean message) {
        if (p.getSkillManager().getCurrentLevel(Skill.FISHING) < fish.getRequiredLevel()) {
            p.getPacketSender().sendMessage("You need a fishing level of " + fish.getRequiredLevel() + " to fish here.");
            return false;
        }
    ...
    public static boolean hasFishingItems(Player player, FishableData.Fishable fish, boolean message) {
        ...
        if (tool == 311) {
            if(!player.getInventory().contains(new Item(tool, 1))) {
                ...
                if (message) {
                    player.getPacketSender().sendMessage("You don't have the right tool to fish here.");
                }
                return false;
            }
        } else if (!player.getInventory().contains(new Item(tool, 1))) {
            //TODO: ADD STUFF HERE FOR IT TO WORK PROPERLY
            player.getPacketSender().sendMessage("You need a proper tool to fish here.")
            return false;
        }

        if ((bait > -1) && (!player.getInventory().contains(new Item(bait, 1)))) {
            //String name = Item.getDefinition(bait).getName();
            player.getPacketSender().sendMessage("You need Proper bait to fish here")
            return false;
        }
    ...
    private FishToolData.Tools tool = null;

    public Fishing(Player player) {
        this.player = player;
    }

    public boolean clickNpc(Mob mobd, int id, int option) {
